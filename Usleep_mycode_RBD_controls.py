...
from My_functions_script_RBD_controls import extract_numbers_from_filename, extract_letters_and_numbers, list_files_in_folder, split_string_by_length, Usleep_2channels, correlation_multiple_electrodes
...
####### Setting paths ##################################
# File paths 
input_path =r'/scratch/users/s184063/RBD_controls_Restructure_firsttry/'
#input_file=f'A0001_4 165907.EDF' # Easier later for namechange according to the patient ID 
#input_file_path = os.path.join(input_path,input_file)

output_path = r'/scratch/users/s184063/hypnograms_RBD_controls_firsttry/' 
#output_path = r'/scratch/users/s184063/hypnograms trial RBD controls/' 
...
time_signal_folder=r'/scratch/users/s184063/RBD_controls_Restructure_firsttry/'
...
E6_epoc1=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_6E_epocssize_1_RBD_controls.csv')
E6_epoc3=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_6E_epocssize_3_RBD_controls.csv')
E6_epoc5=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_6E_epocssize_5_RBD_controls.csv')
E6_epoc15=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_6E_epocssize_15_RBD_controls.csv')
E6_epoc30=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_6E_epocssize_30_RBD_controls.csv')
...
df_6E.to_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_6E_RBD_controls_all_epocs.csv')


# Loading CSV files for 2-electrode data for all epocs
E5_epoc1=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_5E_epocssize_1_RBD_controls.csv')
E5_epoc3=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_5E_epocssize_3_RBD_controls.csv')
E5_epoc5=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_5E_epocssize_5_RBD_controls.csv')
E5_epoc15=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_5E_epocssize_15_RBD_controls.csv')
E5_epoc30=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_5E_epocssize_30_RBD_controls.csv')


# Dropping the first patienID folder for most of the files in order to make a concatenation of all files 
E5_epoc3=E5_epoc3.drop('PatientID',axis=1)
E5_epoc5=E5_epoc5.drop('PatientID',axis=1)
E5_epoc15=E5_epoc15.drop('PatientID',axis=1)
E5_epoc30=E5_epoc30.drop('PatientID',axis=1)


# Combining the dataframes. Only one column contains patientID
df_5E=pd.concat([E5_epoc1,E5_epoc3,E5_epoc5,E5_epoc15,E5_epoc30],axis=1)
df_5E.to_csv('/scratch/users/s184063/RBD_controls_Features/Correlation_5E_RBD_controls_all_epocs.csv')


# Combine CSV files sorting by the electrodes - There will be many NaN values 
df_combined=pd.concat([df_6E, df_5E])
df_combined.to_csv('/scratch/users/s184063/RBD_controls_Features/Combined_RBD_controls.csv')

####################################################################################################
# Extract age, sex, and diagnosis label (Dx)
demographics=pd.read_csv('/scratch/users/s184063/stanford_irbd_control_ds.csv')
print('demographic features for use')
Demographic_features_for_use=demographics[['ID','Age','Sex_F1_M2','Dianosis']] # Dx is the label (RBD or control)
...
patientID_temp=df_combined['PatientID'] # Extracting patientID's 
df_combined['PatientID']=patientID_temp+'.edf'
patientID=df_combined['PatientID']
...
temporary_df=pd.DataFrame({'ID':patientID})
print('Temporary df')
print(temporary_df)

# Merging and extratcing the features with overlapping patientIDs and cohorts 
result_df=pd.merge(Demographic_features_for_use,temporary_df,how='inner',on=['ID'])
print('Results df - first time')
print(result_df)

# Changing name back to 'patientID'
result_df=result_df.rename(columns={'ID':'PatientID'})
...
result_final_df.to_csv('/scratch/users/s184063/RBD_controls_Features/All_features_combined_RBD_controls.csv')
...
#Loading data frame 
df_combined=pd.read_csv('/scratch/users/s184063/RBD_controls_Features/All_features_combined_RBD_controls.csv')
...
matrix.to_csv('/scratch/users/s184063/RBD_controls_Features/Matrix_correlation_RBD_controls.csv')
...
# Display the plot
plt.savefig('/scratch/users/s184063/RBD_controls_Features/Correlation_matrix_RBD_controls.png')
...
mean.to_csv('/scratch/users/s184063/RBD_controls_Features/Mean values.csv')
std=pd.DataFrame({'std':std_stack})
std.to_csv('/scratch/users/s184063/RBD_controls_Features/Std values.csv')
min_val=pd.DataFrame({'minimum':min_stack})
min_val.to_csv('/scratch/users/s184063/RBD_controls_Features/Min values.csv')
max_val=pd.DataFrame({'max':max_stack})
max_val.to_csv('/scratch/users/s184063/RBD_controls_Features/Max values.csv')
...
for d in range(len(E_combinations)):
    ...
    for i in wake:
        # Wake   
        plt.scatter(np.array(df_combined['Age']),np.array(df_combined[i]))
    plt.xlabel('Age')
    ...
    for j in N1:
        plt.scatter(np.array(df_combined['Age']),np.array(df_combined[j]))
        #print(df_combined[j])
    plt.xlabel('Age')
    ...
    for k in N2:
        plt.scatter(np.array(df_combined['Age']),np.array(df_combined[k]))
    plt.xlabel('Age')
    ...
    for h in N3:
        ...
        plt.scatter(np.array(df_combined['Age']),np.array(df_combined[h]))
    plt.xlabel('Age')
    ...
    for dd in REM:
        ...
        plt.scatter(np.array(df_combined['Age']),np.array(df_combined[dd]))
    plt.xlabel('Age')
    ...
    plt.savefig('/scratch/users/s184063/RBD_controls_Features/Scatter_RBD_'+str(Electrode_combination)+'.png')
...
for d in range(len(E_combinations)):
    ...
    print('Figure was saved')
    plt.savefig('/scratch/users/s184063/RBD_controls_Features/Boxplot_RBD_'+str(Electrode_combination)+'.png')
