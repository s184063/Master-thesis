...
from My_functions_script_Glostrup import extract_numbers_from_filename, extract_letters_and_numbers, list_files_in_folder, split_string_by_length, Usleep_2channels, correlation_multiple_electrodes
...
####### Setting paths ##################################
# File paths 
input_path =r'/scratch/users/s184063/Glostrup_Restructure_firsttry/'
#input_file=f'A0001_4 165907.EDF' # Easier later for namechange according to the patient ID 
#input_file_path = os.path.join(input_path,input_file)
output_path = r'/scratch/users/s184063/hypnograms_Glostrup_firsttry/' 
#######################################################
...
for edf_file in edf_files_list:
    ...
    # For naming in the Glostrup dataset include both names and numbers
...
# In the end a major file for the Glostrup cohort will be gathered and ready for modelling. 

# Combining CSV files 

# Loading CSV files for 2-electrode data for all epocs
E6_epoc1=pd.read_csv('/scratch/users/s184063/Glostrup_Features/Correlation_6E_epocssize_1_Glostrup.csv')
E6_epoc3=pd.read_csv('/scratch/users/s184063/Glostrup_Features/Correlation_6E_epocssize_3_Glostrup.csv')
E6_epoc5=pd.read_csv('/scratch/users/s184063/Glostrup_Features/Correlation_6E_epocssize_5_Glostrup.csv')
E6_epoc15=pd.read_csv('/scratch/users/s184063/Glostrup_Features/Correlation_6E_epocssize_15_Glostrup.csv')
E6_epoc30=pd.read_csv('/scratch/users/s184063/Glostrup_Features/Correlation_6E_epocssize_30_Glostrup.csv')
...
df_6E.to_csv('/scratch/users/s184063/Glostrup_Features/Correlation_6E_Glostrup_all_epocs.csv')
...
Cohort_fake=pd.Series('DHC', index=patientID.index)
...
# Final document containing all features of interest including the correlation features for Glostrup DHC
result_final_df=pd.merge(result_df,df_6E,how='inner',on=['PatientID'])
result_final_df.to_csv('/scratch/users/s184063/Glostrup_Features/All_features_combined_Glostrup.csv')
print('Final results')
...
# Loading data frame 
df_combined=pd.read_csv('/scratch/users/s184063/Glostrup_Features/All_features_combined_Glostrup.csv')
...
print(matrix)
matrix.to_csv('/scratch/users/s184063/Glostrup_Features/Matrix_correlation_controls.csv')
...
# Adding the title
plt.title('Glostrup Controls - Correlation matrix of correlated hypnodensity features')

# Adjust the padding
plt.tight_layout()

# Display the plot
plt.savefig('/scratch/users/s184063/Glostrup_Features/Correlation_matrix_Glostrup_Controls.png')
...
Y = df_NT1['age_vst'] # replace 'Y' with your actual column name for the dependent variable
X = df_NT1.iloc[:,24] # differ this variable to select single features to compare with age 
...
mean.to_csv('/scratch/users/s184063/Glostrup_Features/Mean values.csv')
std=pd.DataFrame({'std':std_stack})
std.to_csv('/scratch/users/s184063/Glostrup_Features/Std values.csv')
min_val=pd.DataFrame({'minimum':min_stack})
min_val.to_csv('/scratch/users/s184063/Glostrup_Features/Min values.csv')
max_val=pd.DataFrame({'max':max_stack})
max_val.to_csv('/scratch/users/s184063/Glostrup_Features/Max values.csv')
...
for d in range(len(E_combinations)):
    ...
    plt.legend(['1s','3s','5s','15s','30s'])
    plt.tight_layout()
    plt.suptitle(" Glostrup NT1- Scatterplot of sleep stages vs. age - "+str(Electrode_combination))
    plt.tight_layout()
    
    print('Figure was saved')
    plt.savefig('/scratch/users/s184063/Glostrup_Features/Scatter_NT1_Glostrup_'+str(Electrode_combination)+'.png')
...
for d in range(len(E_combinations)):
    ...
    plt.legend(['1s','3s','5s','15s','30s'])
    plt.tight_layout()
    plt.suptitle(" Glostrup Controls- Scatterplot of sleep stages vs. age - "+str(Electrode_combination))
    plt.tight_layout()
    
    print('Figure was saved')
    plt.savefig('/scratch/users/s184063/Glostrup_Features/Scatter_Controls_Glostrup_'+str(Electrode_combination)+'.png')
...
for d in range(len(E_combinations)):
    ...
    print('Figure was saved')
    plt.savefig('/scratch/users/s184063/Glostrup_Features/Boxplot_NT1_Glostrup_'+str(Electrode_combination)+'.png')
...
for d in range(len(E_combinations)):
    ...
    plt.tight_layout()
    print('Figure was saved')
    plt.savefig('/scratch/users/s184063/Glostrup_Features/Boxplot_controls_Glostrup_'+str(Electrode_combination)+'.png')
    plt.clf()
    del boxplot_features
...
'''
Electrode_combination='C3M2F4M1'
# for all features 
#plt.figure(figsize=(8,6)) # Adjust these numbers as per your requirement.
wake=['Wake_'+str(Electrode_combination)+'_epocssize_1', 'N1_'+str(Electrode_combination)+'_epocssize_1', 'N2_'+str(Electrode_combination)+'_epocssize_1', 'N3_'+str(Electrode_combination)+'_epocssize_1', 'REM_'+str(Electrode_combination)+'_epocssize_1']

plt.subplot(2, 3, 1) # (dimension1, dimension2, number of plots)
for i in range(len(wake)):

    sns.histplot(data=df_NT1[i])
    #plt.xticks(rotation=45, ha='right')
    #plt.suptitle('Histogram - Epoch size 30s for all plots')
    plt.tight_layout()
    print('Figure was saved')
    plt.savefig('/scratch/users/s184063/Glostrup_Features/Histogram_Glostrup_'+str(Electrode_combination)+'.png')
    plt.clf()


plt.subplot(2,3,2)
sns.histplot(data=df_NT1.iloc[:,21])
#plt.xticks(rotation=45, ha='right')
plt.suptitle('Histogram - Epoch size 30s for all plots')
plt.tight_layout()

plt.subplot(2,3,3)
sns.histplot(data=df_NT1.iloc[:,22])
#plt.xticks(rotation=45, ha='right')
plt.suptitle('Histogram - Epoch size 30s for all plots')
plt.tight_layout()

plt.subplot(2,3,4)
sns.histplot(data=df_NT1.iloc[:,23])
#plt.xticks(rotation=45, ha='right')
plt.suptitle('Histogram - Epoch size 30s for all plots')
plt.tight_layout()
...
plt.subplot(2,3,5)
sns.histplot(data=df_NT1.iloc[:,24])
#plt.xticks(rotation=45, ha='right')
plt.suptitle('Histogram - Epoch size 30s for all plots')
plt.tight_layout()
'''
