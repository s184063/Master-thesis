...
# Input path 
path_edf=r'/scratch/users/s184063/RBD_controls_subsample/'
...
for edf_file in edf_files_list:
    ...
    for k in range(len(signal_headers)):
        ...
        print('Temporary electrode')
        print(temp_electrodes)
        ...
        for i in range(0,len(time_signal),time_30s_calculate):
            ...
            if 170*abs(overall_median) <= median_30sec and 8*overall_std <= std_30sec:
            ...
            # Checking if the signal are out of range 
            elif 170*overall_median <= median_30sec and 8*overall_std <= std_30sec:
                # The factors multiplied on the median and std are found by trial and error 
                # This part checks for large positive amplitudes in the signal 
                out_of_range='True'
                temp_out_of_range.append(out_of_range)
                #Outlier=np.stack(temp_out_of_range,axis=0)
                print('Negative median and std was used to detect an outlier')

            elif abs(spectrum_median_30sec) < 0.1*overall_spectrum_median and abs(spectrum_std_30sec) < 0.1*overall_spectrum_std:
        ...
        print(temp_out_of_range)
        print(len(temp_out_of_range))
        ...
        for loop_factor in signal_index:
            ...
            matplotlib.pyplot.savefig(f'/scratch/users/s184063/RBD_controls_Features/Outlier_number_{loop_factor}.png')
            # Clearing plot
...
if 'bad_channels_for_csv' in locals():
    ...
    full_dataframe.to_csv('/scratch/users/s184063/RBD_controls_Features/Outliers_trial.csv',index=False)
else: 
