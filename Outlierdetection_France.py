...
for edf_file in edf_files_list:
    ...
    # PatientID !!!!!!!!!!!! Change this when the folder changes !!!!!!!!!!!!!
    file_temp=str(edf_file) # /scratch/users/s184063/France restructure EDF all correct final/restructuredfile_France_all_050216AA.EDF
    filename=file_temp[64:]
    print('Filename')
    print(filename)
...
for edf_file in edf_files_list:
    ...
    for k in range(len(signal_headers)):
        ...
        for i in range(0,len(time_signal),time_30s_calculate):
            ...
            if 14*abs(overall_median) <= median_30sec and 2*overall_std <= std_30sec:
            ...
            # Checking if the signal are out of range 
            elif 14*overall_median <= median_30sec and 2*overall_std <= std_30sec:
                # The factors multiplied on the median and std are found by trial and error 
                # This part checks for large positive amplitudes in the signal 
                out_of_range='True'
                temp_out_of_range.append(out_of_range)
        ...
        for loop_factor in signal_index:
            ...
            matplotlib.pyplot.savefig(f'/scratch/users/s184063/France subsample/Outlier_number_{loop_factor}.png')
