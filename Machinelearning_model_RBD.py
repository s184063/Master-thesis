...
print(y_pred_concatenated)
print(len(y_pred_concatenated))
...
random_seed=np.random.RandomState(seed=42)
X_train, X_test, y_train, y_test = train_test_split(X_matrix, Y_variable, test_size=0.2,random_state=42)
# testsize = 20% 
# train size = 80% 


print('Information about x-train')
print(X_train.shape)
print(X_train)

print('Information about x-test')
print(X_test.shape)
print(X_test)

print('Information about y-train')
print(y_train.shape)
print(y_train)

print('Information about y-test')
print(y_test.shape)
print(y_test)
print(type(y_test))
...
# 4) feature importance on 5 models - take average later 
...
display = PrecisionRecallDisplay.from_estimator(
    bst, X_test, y_test, name="BostedDecisionTree", plot_chance_level=True
)
_ = display.ax_.set_title("2-class Precision-Recall curve, NT1 model France")
...
for i, importance in enumerate(importances):
    ...
    if importance > 0:
        # Indexing in X to find the name of the features 
        column_name = X_matrix.columns[i]
...
### Permutation on the test set #######
result = permutation_importance(
    bst, X_test, y_test, n_repeats=10, random_state=42, n_jobs=2
)

sorted_importances_idx = result.importances_mean.argsort()
importances = pd.DataFrame(
    result.importances[sorted_importances_idx].T,
    columns=X_matrix.columns[sorted_importances_idx],
)
print('Pertumation test set')
print(importances)
ax = importances.plot.box(vert=False, whis=10)
ax.set_title("Permutation Importances (test set), NT1 model France")
ax.axvline(x=0, color="k", linestyle="--")
ax.set_xlabel("Decrease in accuracy score")
ax.figure.tight_layout()
plt.show()
...
### Permutation training set 
result = permutation_importance(
    bst, X_train, y_train, n_repeats=10, random_state=42, n_jobs=2
)
...
print('Pertumation train set')
print(importances)
ax = importances.plot.box(vert=False, whis=10)
ax.set_title("Permutation Importances (train set), NT1 model France")
ax.axvline(x=0, color="k", linestyle="--")
ax.set_xlabel("Decrease in accuracy score")
ax.figure.tight_layout()
plt.show()
...
preds = bst.predict(X_test) # output probabilities 

# threshold predictions before inputting to metrics 
...
accuracy_metric=sklearn.metrics.accuracy_score(y_test,preds) # y_true, y_predicted
print('Accuracy')
print(accuracy_metric)

f1_score_metric=sklearn.metrics.f1_score(y_test,preds)
print('f1-score')
print(f1_score_metric)

confusion_matrix_metrics=sklearn.metrics.confusion_matrix(y_test,preds)
print('Confusion matrix')
print(confusion_matrix_metrics)
